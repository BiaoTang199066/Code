...
#y=c(2.71,2.44,2.69,3.31,3.44,3.52)
x=c(1.724624,1.622481,1.599018,1.490909,1.478627,1.066138)
...
y=c(3.04, 3.22, 2.44, 3.34, 3.52, 3.22,3.46, 3.4, 3.7)
z=c(0.34, 0.37, 0.24, 0.39, 0.42, 0.37, 0.45, 0.41, 0.4)
cor.test(x,y,alternative='less')
...
abline(lm(y~x),lwd=2)
corp=cor(x,y)
text(2,2.6,as.expression(substitute(Cor[p]==corp,list(corp=round(corp,2)))))
cors=cor(x,y,method='spearman')
text(2,2.5,as.expression(substitute(Cor[s]==cors,list(cors=round(cors,2)))))
